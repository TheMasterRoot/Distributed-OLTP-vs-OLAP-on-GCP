...
const translations: Record<Language, Record<string, string>> = {
  ...
  pt: {
    ...
    'Why OLTP and OLAP Exist': 'Por Que OLTP e OLAP Existem',
    'Start with the workload': 'Comece pelo workload',
    'The database is not the strategy. The workload is.': 'O banco de dados não é a estratégia. O workload é.',
    'OLTP and OLAP solve different business questions. One protects the transaction while it happens; the other explains what happened across millions or billions of events.':
      'OLTP e OLAP respondem perguntas de negócio diferentes. Um protege a transação enquanto ela acontece; o outro explica o que aconteceu em milhões ou bilhões de eventos.',
    'Guiding question': 'Pergunta-guia',
    'Am I changing the state of the business or analyzing its history?':
      'Estou mudando o estado do negócio ou analisando sua história?',
    '1. Operational truth': '1. Verdade operacional',
    'Orders, payments, inventory, login sessions, account balances. Small writes must be correct now.':
      'Pedidos, pagamentos, estoque, sessões de login, saldos de conta. Pequenas escritas precisam estar corretas agora.',
    '2. Movement without damage': '2. Movimento sem dano',
    'CDC and streaming move facts out of production so analytics do not steal CPU, locks, or IOPS from users.':
      'CDC e streaming tiram fatos da produção para que analytics não roube CPU, locks ou IOPS dos usuários.',
    '3. Analytical memory': '3. Memória analítica',
    'Revenue trends, cohorts, fraud signals, forecasting, dashboards. Big scans are expected, not accidental.':
      'Tendências de receita, coortes, sinais de fraude, previsões, dashboards. Grandes scans são esperados, não acidentais.',
    '4. Architecture decision': '4. Decisão de arquitetura',
    'Cloud SQL, Spanner, AlloyDB, and BigQuery are not interchangeable. Each optimizes a different shape of work.':
      'Cloud SQL, Spanner, AlloyDB e BigQuery não são intercambiáveis. Cada um otimiza um formato diferente de trabalho.',
    'OLTP vs OLAP: What Each One Is Good At': 'OLTP vs OLAP: No Que Cada Um É Bom',
    'Run the business': 'Operar o negócio',
    'Fast, consistent transactions for operational systems.': 'Transações rápidas e consistentes para sistemas operacionais.',
    'Good at': 'Bom em',
    'checkout, payments, stock updates, user actions': 'checkout, pagamentos, atualização de estoque, ações de usuários',
    'Optimized for': 'Otimizado para',
    'low latency, ACID, many small reads and writes': 'baixa latência, ACID, muitas leituras e escritas pequenas',
    'Watch out for': 'Cuidado com',
    'big scans, ad hoc reports, dashboard traffic': 'grandes scans, relatórios ad hoc, tráfego de dashboards',
    'Best fit: Cloud SQL, AlloyDB, Spanner': 'Melhor encaixe: Cloud SQL, AlloyDB, Spanner',
    'Workload Signal': 'Sinal do Workload',
    'Latency sensitive': 'Sensível a latência',
    'Transactional writes': 'Escritas transacionais',
    'Historical scans': 'Scans históricos',
    'Aggregations': 'Agregações',
    'Concurrent users': 'Usuários concorrentes',
    'OLTP / OLAP': 'OLTP / OLAP',
    'OLTP thinks in latency': 'OLTP pensa em latência',
    'OLAP thinks in volume': 'OLAP pensa em volume',
    'ms: OLTP thinks in latency': 'ms: OLTP pensa em latência',
    'TB: OLAP thinks in volume': 'TB: OLAP pensa em volume',
    'Understand the business': 'Entender o negócio',
    'Large-scale analysis for decisions, reporting, and discovery.':
      'Análise em larga escala para decisões, relatórios e descoberta.',
    'dashboards, cohorts, forecasting, fraud analysis': 'dashboards, coortes, previsões, análise de fraude',
    'column scans, joins, aggregates, cheap storage': 'scans colunares, joins, agregações, storage barato',
    'single-row updates, high-frequency transactions': 'updates de uma linha, transações de alta frequência',
    'Best fit: BigQuery, Looker, BI Engine': 'Melhor encaixe: BigQuery, Looker, BI Engine',
  ...
  es: {
    ...
    'Why OLTP and OLAP Exist': 'Por Qué Existen OLTP y OLAP',
    'Start with the workload': 'Empieza por el workload',
    'The database is not the strategy. The workload is.': 'La base de datos no es la estrategia. El workload sí.',
    'OLTP and OLAP solve different business questions. One protects the transaction while it happens; the other explains what happened across millions or billions of events.':
      'OLTP y OLAP responden preguntas de negocio diferentes. Uno protege la transacción mientras ocurre; el otro explica lo que pasó en millones o miles de millones de eventos.',
    'Guiding question': 'Pregunta guía',
    'Am I changing the state of the business or analyzing its history?':
      '¿Estoy cambiando el estado del negocio o analizando su historia?',
    '1. Operational truth': '1. Verdad operacional',
    'Orders, payments, inventory, login sessions, account balances. Small writes must be correct now.':
      'Órdenes, pagos, inventario, sesiones de login, saldos de cuenta. Las escrituras pequeñas deben ser correctas ahora.',
    '2. Movement without damage': '2. Movimiento sin daño',
    'CDC and streaming move facts out of production so analytics do not steal CPU, locks, or IOPS from users.':
      'CDC y streaming sacan hechos de producción para que analytics no robe CPU, locks o IOPS a los usuarios.',
    '3. Analytical memory': '3. Memoria analítica',
    'Revenue trends, cohorts, fraud signals, forecasting, dashboards. Big scans are expected, not accidental.':
      'Tendencias de ingresos, cohortes, señales de fraude, pronósticos, dashboards. Los grandes scans son esperados, no accidentales.',
    '4. Architecture decision': '4. Decisión de arquitectura',
    'Cloud SQL, Spanner, AlloyDB, and BigQuery are not interchangeable. Each optimizes a different shape of work.':
      'Cloud SQL, Spanner, AlloyDB y BigQuery no son intercambiables. Cada uno optimiza una forma distinta de trabajo.',
    'OLTP vs OLAP: What Each One Is Good At': 'OLTP vs OLAP: En Qué Es Bueno Cada Uno',
    'Run the business': 'Operar el negocio',
    'Fast, consistent transactions for operational systems.': 'Transacciones rápidas y consistentes para sistemas operacionales.',
    'Good at': 'Bueno en',
    'checkout, payments, stock updates, user actions': 'checkout, pagos, actualización de stock, acciones de usuarios',
    'Optimized for': 'Optimizado para',
    'low latency, ACID, many small reads and writes': 'baja latencia, ACID, muchas lecturas y escrituras pequeñas',
    'Watch out for': 'Cuidado con',
    'big scans, ad hoc reports, dashboard traffic': 'grandes scans, reportes ad hoc, tráfico de dashboards',
    'Best fit: Cloud SQL, AlloyDB, Spanner': 'Mejor encaje: Cloud SQL, AlloyDB, Spanner',
    'Workload Signal': 'Señal del Workload',
    'Latency sensitive': 'Sensible a latencia',
    'Transactional writes': 'Escrituras transaccionales',
    'Historical scans': 'Scans históricos',
    'Aggregations': 'Agregaciones',
    'Concurrent users': 'Usuarios concurrentes',
    'OLTP / OLAP': 'OLTP / OLAP',
    'OLTP thinks in latency': 'OLTP piensa en latencia',
    'OLAP thinks in volume': 'OLAP piensa en volumen',
    'ms: OLTP thinks in latency': 'ms: OLTP piensa en latencia',
    'TB: OLAP thinks in volume': 'TB: OLAP piensa en volumen',
    'Understand the business': 'Entender el negocio',
    'Large-scale analysis for decisions, reporting, and discovery.':
      'Análisis a gran escala para decisiones, reportes y descubrimiento.',
    'dashboards, cohorts, forecasting, fraud analysis': 'dashboards, cohortes, pronósticos, análisis de fraude',
    'column scans, joins, aggregates, cheap storage': 'scans columnares, joins, agregaciones, storage barato',
    'single-row updates, high-frequency transactions': 'updates de una fila, transacciones de alta frecuencia',
    'Best fit: BigQuery, Looker, BI Engine': 'Mejor encaje: BigQuery, Looker, BI Engine',
